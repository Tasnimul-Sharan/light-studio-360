...
export default function NewsDetails({ blogsData, socialIcons }) {
        ...
        <div className="space-y-4 text-gray-700 leading-relaxed text-base">
          {Array.isArray(blogPost.subContent) ? (
            ...
            <div className="flex items-start gap-3">
              <FaCheckCircle className="text-primary mt-1" />
              <p className="flex-1">{blogPost.subContent}</p>
            </div>
          )}
        </div>
      )}
            ...
            {socialIcons?.map(({ icon: Icon, link }, index) => {
              ...
              return (
                <a
                  ...
                  className="w-8 h-8 flex items-center justify-center border border-textSecondary text-textSecondary p-2 rounded-full hover:bg-primary hover:text-white transition-all duration-500"
                >
